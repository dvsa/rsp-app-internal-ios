...
if [ "$unittests" = "on" ]; then
    ...
    buildCmd=(xcodebuild build test)
    if [[ ! -z "$workspaceFile" ]]; then
        buildCmd+=(-workspace $workspaceFile)
    elif [[ ! -z "$projectFile" ]]; then
	      buildCmd+=(-project $projectFile)
    fi
    buildCmd+=( -scheme "$appScheme" -configuration "$appConfiguration" -enableCodeCoverage YES ENABLE_TESTABILITY=YES)
    if [[ ! -z "$destinationSimulator" ]]; then
        buildCmd+=(-destination "$destinationSimulator" -destination-timeout 900)
    fi
